...
export default function InnovativeSignups({ signups }: { signups: any[] }) {
  ...
  return (
    <div className="w-full flex flex-col md:flex-row overflow-hidden shadow-xl">
      ...
      <div className="flex flex-col justify-start items-start w-full md:w-1/4 min-w-0 bg-gradient-to-br from-blue-500 via-blue-400 to-blue-300 dark:from-blue-600 dark:via-blue-500 dark:to-blue-400 pt-6 pb-2 px-4 md:pt-8 md:pb-0 md:px-6 ">
        <h3 className="text-gray-900 text-white text-lg sm:text-xl md:text-2xl font-bold mb-1 md:mb-2 drop-shadow dark:drop-shadow-none">
          Son Kayıtlar
        </h3>
        <p className="text-gray-800/90 dark:text-white/90 text-white text-xs sm:text-sm md:text-base font-medium mb-1">
          Sunucumuza kayıt olan son <span className="font-bold">{total}</span>{" "}
          oyuncu!
        </p>
      </div>
      {/* Sağ Panel */}
      <div className="w-full md:w-3/4 flex items-center justify-center px-2 md:px-6 py-4 md:py-6 gap-2 md:gap-8 bg-white dark:bg-[#0a1121] transition-colors">
        {!displaySignups || displaySignups.length === 0 ? (
          <p className="text-center text-xs sm:text-sm text-gray-500 dark:text-gray-300">
            Son zamanlarda yeni katılan olmadı.
          </p>
        ) : (
          <div className="flex gap-2 md:gap-10 items-center w-full justify-center">
            {displaySignups.map((signup, idx) => (
              ...
              >
                ...
                <Head username={signup.username} size={40} className="shadow" />
